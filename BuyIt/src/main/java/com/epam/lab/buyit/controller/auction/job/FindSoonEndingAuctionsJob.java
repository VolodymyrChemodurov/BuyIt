...
public class FindSoonEndingAuctionsJob implements Job {
	...
	private static final Logger LOGGER = Logger.getLogger(FindSoonEndingAuctionsJob.class);
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		List<Auction> soonEndingAuctions = auctionService.getSoonEndingAuctions(currentTime, endTime);
		...
		for (Auction currentAuction : soonEndingAuctions) {
			...
			Trigger trigger = creatorDisposable.create(currentAuction .getEndTime().getTime());
			trigger.getJobDataMap().put("auctionId", currentAuction.getIdAuction());
			try {
				scheduler.scheduleJob(serveAuctionJobCreator.create(), trigger);
				StringBuilder infoString = new StringBuilder("Put auction id = ")
					.append(currentAuction.getIdAuction())
